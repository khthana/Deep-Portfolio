...
และเป็นส่วนที่เคยค้างอยู่ของ T5 ใน
[`spec-refactor-redeploy.md`](spec-refactor-redeploy.md) ซึ่งปิดไปพร้อมกับตารางนี้
...
| TC-30 | เพิ่มกิจกรรมการประเมินโดยกรอกข้อมูลไม่ครบ | ครอบคลุมบางส่วน | `activity.test.ts` — `POST /activity` "answers 400 when the request carries no rubric", "answers 400 for a kind of work it does not have", "answers 400 for a rubric that is not JSON, and uploads nothing", "answers 400 for two levels sent at the same position" | ตามกติกาข้อ 2 |
| TC-31 | แก้ไขกิจกรรมการประเมิน | ครอบคลุมแล้ว | `activity.test.ts` — `PUT /activity` "updates the activity and replaces its rubric", "keeps the marks already given when the rubric comes back untouched", "rewrites a kept criterion in place, wording and weight alike", "adds a criterion without disturbing the one already marked", "drops the marks of a criterion the teacher deleted, and only those", "drops a mark given at a level the teacher deleted", "keeps a mark on the level it was given when the form renumbers", "answers 400 for the same criterion sent twice", "answers 400 for a criterion that belongs to another activity", "answers 400 for the same level sent twice", "answers 400 for a level belonging to another criterion", "answers 400 for two levels sent at the same position" | [#25](https://github.com/khthana/Deep-Portfolio/issues/25) แก้แล้ว — เกณฑ์ที่ส่ง `id` กลับมาถูกเขียนทับที่เดิม คะแนนที่ตรวจไปแล้วจึงอยู่ครบ ส่วนการเรียงเลข `level_no` ใหม่ตอนลบช่วงคะแนน [#39](https://github.com/khthana/Deep-Portfolio/issues/39) แก้แล้วเช่นกัน — ช่วงคะแนนส่ง `id` ของตัวเองกลับมา |
...
| TC-33 | ดูภาพรวมการส่งงาน | ครอบคลุมแล้ว | `activity.test.ts` — `GET /activity/submitted/list` "returns everyone the activity was set for, those who handed something in first", "lists a group that has handed nothing in, and who it is waiting on", "lists a student who is in no group at all as a row of their own", "puts a group that handed something in ahead of one that did not"; `GET /activity/student/detail` "returns one student's submission alongside the activity itself" | [#56](https://github.com/khthana/Deep-Portfolio/issues/56) แก้แล้ว — "ภาพรวม" เดิมมีแต่คนที่ส่งแล้ว ตอนนี้มีทุกคนที่ถูกสั่งงาน |
| TC-34 | ตรวจงานด้วย rubric พร้อมให้ feedback | ครอบคลุมแล้ว | `student-activity.test.ts` — `POST /student-activity/grade` "scores an individual submission from the levels it is given", "replaces the levels a re-marked submission was given before", "writes the student's share of every CLO the activity is mapped to", "gives every member of a group the same score", "passes over a member who never answered the invitation" | การคำนวณคะแนนและการเขียนคะแนนกลับเข้า CLO ยืนยันด้วยตัวเลขจริงในฐานข้อมูล |
...
| TC-39 | ตรวจกิจกรรมการเรียนรู้ | ครอบคลุมแล้ว | `student-learning-activity.test.ts` — `POST /student-learning-activity/grade` "marks an individual submission graded with the feedback given", "marks every member of a group graded at once", "passes over a member who never answered the invitation" | |
...
ไปตั้งแต่ต้น ไม่ใช่ช่องว่างที่ตั้งตั๋วแล้วจะมีคนปิดได้ — ตั๋วที่ถามว่า**ควรเพิ่ม
รอยต่อที่สามหรือไม่** มีแล้วที่
[#62](https://github.com/khthana/Deep-Portfolio/issues/62) ตั้งแต่ 12 สิงหาคม 2569
ถ้าคำตอบคือเพิ่ม สี่ข้อนี้คือของที่จะได้ครบเป็นข้อแรก ๆ
