...
| TC | เรื่อง | สถานะ | test ที่ครอบ | หมายเหตุ |
| --- | --- | --- | --- | --- |
| TC-42 | สร้างกลุ่มสำหรับงานกลุ่ม | ครอบคลุมแล้ว | `activity-group.test.ts` — `POST /student-activity-group` "creates the group, its members and a submission row for each", "reuses a submission row the student already had"; `PATCH /student-activity-group` "adds a member and keeps the answers the existing ones already gave", "removes a member left out of the new list" | สาม `GET` ที่หน้าจอนี้ใช้อ่านกลุ่ม อ่านนักศึกษาจาก session แล้วตั้งแต่ [#26](https://github.com/khthana/Deep-Portfolio/issues/26) — case "answers about the caller, whoever the request names" และ "refuses a student who is not enrolled in the section" ทั้งสองไฟล์ ดู [ADR-0003](adr/0003-enrolment-access.md) ส่วนการตรวจว่าผู้เรียกอยู่ในกลุ่มที่ตัวเองสร้างหรือแก้จริง ยังค้างที่ [#27](https://github.com/khthana/Deep-Portfolio/issues/27) |
