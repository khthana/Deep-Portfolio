...
| TC-44 | ดูผลการประเมิน | ครอบคลุมแล้ว | `evaluation.test.ts` — `GET /evaluation/list` "lists an announced activity with the student's mark and the class spread", "leaves the class spread empty until somebody has been marked", "shows each student their own mark", "hides work whose announcement date has not arrived" | สถิติของชั้นเรียนที่ endpoint นี้ส่งต่อมา ปิดแล้วใน [#28](https://github.com/khthana/Deep-Portfolio/issues/28) ส่วนงานที่ไม่เคยกรอกวันประกาศผลแล้วไม่ถูกแสดงเลย ยัง pin ไว้ที่ [#29](https://github.com/khthana/Deep-Portfolio/issues/29) |
