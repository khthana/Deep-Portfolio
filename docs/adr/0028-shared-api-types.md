...
`apps/web` เขียน type ที่มิเรอร์ response ของ `apps/api` ไว้เอง 38 ไฟล์ 2,161
บรรทัด (นับ 13 สิงหาคม 2569 จาก `apps/web/src/types/*.ts` และ
`apps/web/src/features/**/types/*.ts`) ฝั่ง API ไม่รู้จักไฟล์เหล่านั้นเลย เวลารูป
...
`CLOResp` ที่เหลืออีก 38 ไฟล์อยู่ใน
...
ทั้งสามจุดอ่าน `clo_number`/`clo_detail`/`plo_id` ราวกับว่าไม่มีวันว่าง แก้ด้วย
`??` โดยเลือกค่าที่พฤติกรรมเดิมให้ผลเดียวกันเป๊ะ (`parseInt(null)` กับ
`parseInt("")` ได้ `NaN` เท่ากัน, ข้อความว่างกับ null เรนเดอร์เหมือนกัน, และ `0`
คือค่าที่แถวใหม่ในตาราง CLO ใช้แทน "ยังไม่ผูก PLO" อยู่แล้ว) จึงไม่มีอะไรลง
`BEHAVIOR-CHANGES.md` — ไม่มีสิ่งที่ผู้ใช้เห็นเปลี่ยน

## ผลที่ตามมา

- รูปของ response ของ course มีนิยามเดียวในระบบ และฝั่ง API ผูกไว้กับมันแล้ว
- ยังเหลืออีก 38 ไฟล์ที่ยังเป็นสำเนา — #68 ไล่ทีละ feature, #67 เอา envelope
