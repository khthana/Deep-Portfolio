 ...
 * that stubs out verifyStudent proves the controller works for a request the
 ...
 *
 * Note that a token alone is not authorisation: verifyTeacher and verifyStudent
 * both look the role up in user_roles, so a test that needs one has to insert
 * the row as well.
 */
