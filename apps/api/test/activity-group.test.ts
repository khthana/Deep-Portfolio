...
describe("POST /student-activity-group/resend-invite", () => {
  /**
   * Sending an invitation again — #57.
   *
   * An invite token is good for seven days, and until this endpoint existed
  ...
  it("issues a link that works to a member whose seven days ran out", async () => {
    const group = await createActivityGroup({
      members: [
        {},
        { invite_token: "ran-out", token_expiry: new Date("2020-01-01") },
      ],
    });
    ...
    // Still unanswered: a new link is not an answer to the invitation.
  ...
  it("refuses a member who has already accepted", async () => {
    const group = await createActivityGroup({
      members: [{}, { invite_token: "already-in", status: "ACCEPT" }],
    });
    const [leader, invited] = group.student_activity_group_member;
    ...
    // And the answer they gave stands, token and all.
    expect(
      await prisma.student_activity_group_member.findUniqueOrThrow({
        where: { id: invited.id },
      }),
    ).toMatchObject({ status: "ACCEPT", invite_token: "already-in" });
  });

  it("refuses a member who turned the invitation down", async () => {
    // Whether a refusal may be asked again is a decision for whoever runs the
    // course, not one this endpoint makes quietly (ADR-0025).
    const group = await createActivityGroup({
      members: [{}, { invite_token: "said-no", status: "REJECTED" }],
    });
    const [leader, invited] = group.student_activity_group_member;
  ...
  it("answers 404 for a student who is not in the group", async () => {
    ...
    // The member row is what the whole request acts on, so its absence is a
    // 404 (ADR-0009 §3) — and it tells the leader nothing they cannot already
    // read off their own group.
  ...
  it("refuses a member who is not the leader", async () => {
    const group = await createActivityGroup({ members: [{}, {}, {}] });
    const [, member, other] = group.student_activity_group_member;
