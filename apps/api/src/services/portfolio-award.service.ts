...
type PortfolioAwardWithAttachments = Prisma.portfolio_awardGetPayload<{
  include: {
    portfolio_award_attachments: { include: { attachments: true } };
  };
}>;
...
export default class PortfolioAwardService {
  ...
  async updatePortfolioAward(
    ...
    if (ids_to_delete && ids_to_delete.length > 0) {
      ...
      const objects = await prisma.$transaction(async (tx) => {
        await tx.portfolio_award_attachments.deleteMany({
          where: {
            award_id: id,
  ...
  async deletePortfolioAward(id: number): Promise<PortfolioAwardDetail> {
    const { result, objects } = await prisma.$transaction(async (tx) => {
      ...
      // its own (#34).
      const links = await tx.portfolio_award_attachments.findMany({
        where: { award_id: id },
        select: { attachment_id: true },
      });

      const result = await tx.portfolio_award.delete({
        where: { id },
      });
